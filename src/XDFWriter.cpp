...
bool XDFWriter::Setup(void) {
	...
	// Setup EEG group
	if(this->setup_xdf_group(this->frame_->eeg.info(), 0) == false) {
		std::cerr<<"Cannot setup EEG group"<<std::endl;
		return false;
	}
	
	// Setup EXG group
	if(this->setup_xdf_group(this->frame_->exg.info(), 1) == false) {
		std::cerr<<"Cannot setup EXG group"<<std::endl;
		return false;
	}
	
	// Setup TRI group
	if(this->setup_xdf_group(this->frame_->tri.info(), 2) == false) {
...
bool XDFWriter::Open(const std::string& filename) {
	...
	type = this->get_filetype(filename);
...
int XDFWriter::Write(int nswrite) {
	...
	int nswritten = xdf_write(this->file_, nswrite, 
